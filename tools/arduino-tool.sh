...
print_help()
{
    ...
    echo "${filename} <command, version|init|build|upload> <board, nano|uno|mega> <ino file name> <serial port, /dev/ttyUSB0>"
}
...
check_parameter_1()
{
    if [ $1 != "version" ] && [ $1 != "build" ] && [ $1 != "upload" ] && [ $1 != "init" ]; then
	print_help
	echo "-----------------------------"
	echo "first parameter: only use 'version', 'init', 'build', or 'upload'"
	exit
    fi
}
